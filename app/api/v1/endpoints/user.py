...
@user_router.post("/get_id", response_model=dict)
async def get_user_id(user: UserCreate, db: AsyncSession = Depends(get_session)):
    pass


@user_router.post("/user_info/{user_id}", response_model=dict)
...
@user_router.post("/wall_posts/{user_id}", response_model=dict)
